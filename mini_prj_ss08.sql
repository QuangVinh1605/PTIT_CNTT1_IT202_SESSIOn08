...
select* from rooms where price_per_day >1000000;
 -- cau 5:
 select* from bookings where check_in >= '2024-01-01' and check_in <='2024-12-31' ;
 -- cau 6: 
 ...
 from guests g join bookings b on g.guest_id=b.guest_id group by g.guest_id, b.guest_id; 
 
 -- cau 3: 
 select b.booking_id, g.guest_name, (to_days(b.check_out) - to_days(b.check_in)) * r.price_per_day as doanh_thu
from bookings b join guests g on b.guest_id = g.guest_id join rooms r on b.room_id = r.room_id;

-- cau 4:
select r.room_type, sum((to_days(b.check_out) - to_days(b.check_in)) * r.price_per_day) as tong_doanh_thu
from bookings b join rooms r on b.room_id = r.room_id group by r.room_type;